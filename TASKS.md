...
- [ ] **Add blank lines between connection entries and between the `connections:` and `users:` blocks in `yconn show --dump` output** [cli] S
...
- [ ] **Move SSH config comment metadata into a contiguous block before the `Host` line** [cli] S
